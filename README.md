...
|---|---|---|
| **Claude Code** | только `effort:` во frontmatter агента; у инструмента Agent параметра effort нет | лестница агентов `effort-low…xhigh` + `effort-scout`; гайд в начале сессии; хук `PreToolUse` возвращает модели общий спавн с таблицей, и модель выбирает уровень сама; скил |
...
Нужен Python 3.9+ без сторонних пакетов. Работает на macOS, Linux и Windows: на Windows запускай `python install.py`.
...
python3 install.py               # установить во все найденные CLI (на Windows: python install.py)
```
...
| Codex | копия скила в `~/.codex/skills/effort-routing`; блок между маркерами `<!-- effort-router:begin/end -->` в `~/.codex/AGENTS.md` |
| agy | плагин `~/.gemini/config/plugins/effort-router` (хук, правила, скил) |
...
1. **Уровень выбирает сама модель-оркестратор, у неё есть контекст задачи.** В Claude Code хук `SessionStart` в начале сессии даёт ей короткий гайд: класс задачи → агент нужного effort и модель. В Codex и agy тот же гайд лежит в правилах (`AGENTS.md`). Протокол: дешёвый уровень → проверка тестом или линтером → повтор на одну ступень выше, не больше двух раз.
2. **Хук `PreToolUse` — страховка на случай, когда модель спавнит общего агента** (`general-purpose`, `Explore`, без типа). Как хук поступает, задаёт `hosts.claude.mode` в политике:
   - `ask` (по умолчанию): вызов один раз отклоняется с таблицей в ответе, и модель повторяет его со своим выбором. Если она повторит тот же вызов без изменений, хук сам подберёт ступень по ключевым словам, поэтому цикла не будет;
   - `route`: вызов молча переводится на ступень по ключевым словам, без лишнего хода;
   - `off`: хуки ничего не делают. `EFFORT_ROUTER=off` отключает их на одну сессию.

   Агента, которого модель выбрала сама, хук не трогает. При любой ошибке хук пропускает спавн без изменений. В agy хук переписывает спавн сразу: там выбор сводится к «`flash` (low) или как у родителя».
...
- Хуки Claude Code вызываются как `python3 … || python …`, поэтому подойдёт любой из двух.
- Хук agy запускается через `hook.cmd` с абсолютным путём к интерпретатору, которым запускали установщик.
- Файлы копируются, а не линкуются: символические ссылки на Windows требуют прав.
- Ввод хуков читается строго в UTF-8, независимо от кодовой страницы консоли.
- CI гоняет тесты и установщик на `windows-latest`. Вызов хуков самими CLI проверен только на macOS.
